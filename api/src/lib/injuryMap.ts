import { LikelyInjury } from '../agents/types';
import { MatchSetupContext, RosterPlayerContext } from '../shared/matchContext';
...
const pushUnique = (
  injuries: LikelyInjury[],
  entry: LikelyInjury
): void => {
  if (!injuries.some((item) => item.type.toLowerCase() === entry.type.toLowerCase())) {
    injuries.push(entry);
  }
};
...
export const mapLikelyInjuries = (
